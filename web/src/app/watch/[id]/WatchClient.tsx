...
export default function WatchClient({ id: staticId }: { id: string }) {
  ...
  // Map of titleId -> MutableRefObject<HTMLVideoElement | null>
  const videoRefs = useRef<Map<string, React.MutableRefObject<HTMLVideoElement | null>>>(new Map())
  ...
  // Open the initial tab and fetch title info
  useEffect(() => {
    fetch(`/api/titles/${id}`)
  ...
  // Load subtitle tracks for the active title
  const activeTab = tabs.find((t) => t.id === activeTabId)
  const activeTitleId = activeTab?.titleId ?? id

  useEffect(() => {
    fetch(`/api/titles/${activeTitleId}/subtitles`)
      .then((r) => (r.ok ? r.json() : []))
      .then((tracks: SubTrack[]) => setSubtitles(tracks))
      .catch(() => {})
  }, [activeTitleId])

  // Clean up video refs for removed tabs
  useEffect(() => {
    const tabIds = new Set(tabs.map((t) => t.titleId))
    for (const [tabTitleId] of videoRefs.current) {
      if (!tabIds.has(tabTitleId)) videoRefs.current.delete(tabTitleId)
    }
  }, [tabs])
    ...
    w.__onGCastApiAvailable = (isAvailable: boolean) => {
      ...
      w.cast.framework.CastContext.getInstance().addEventListener(
        ...
        (e: { sessionState: string }) => {
          setCasting(
            e.sessionState === 'SESSION_STARTED' || e.sessionState === 'SESSION_RESUMED'
          )
        }
      )
  ...
  // Get or create a stable ref for a titleId
  function getVideoRef(titleId: string): React.MutableRefObject<HTMLVideoElement | null> {
    if (!videoRefs.current.has(titleId)) {
      videoRefs.current.set(titleId, { current: null })
    }
    return videoRefs.current.get(titleId)!
  }

  // Active video ref helper
  const getActiveVideoRef = useCallback((): React.MutableRefObject<HTMLVideoElement | null> => {
    return videoRefs.current.get(activeTitleId) ?? { current: null }
  }, [activeTitleId])
  ...
  const handleCast = useCallback(() => {
    ...
    const contentType =
      streamMode === 'hls' ? 'application/x-mpegURL' : 'video/mp4'
  ...
  const showControls = useCallback(() => {
    ...
    controlsTimerRef.current = setTimeout(() => {
      ...
      if (activeVideoRef.current && !activeVideoRef.current.paused) setControlsVisible(false)
    }, 3000)
  }, [getActiveVideoRef])
  ...
  const handleSeek = useCallback(
    ...
    [duration, getActiveVideoRef]
  )

  const handleSwitchTab = useCallback(
    (tabId: string) => {
      // Save current time from active video before switching
      const activeVideo = videoRefs.current.get(activeTitleId)?.current
      if (activeVideo && activeTabId) updateCurrentTime(activeTabId, activeVideo.currentTime)
      setActiveTab(tabId)
      // Reset display state for the new active tab
      setCurrentTime(0)
      setDuration(0)
      setPlaying(false)
    },
    [activeTitleId, activeTabId, setActiveTab, updateCurrentTime]
  )
  ...
  const handleCloseTab = useCallback(
    ...
    [closeTab, activeTabId, tabs.length, router]
  )

  const handleAddTab = useCallback(
    (titleId: string, title: string) => {
      openTab({ id: titleId, titleId, title, thumbnail: '' })
      setShowBrowse(false)
    },
    [openTab]
  )
  ...
  return (
    ...
    >
      {/* Breadcrumb */}
      <div
        className={`flex items-center gap-3 px-6 py-3 z-10 bg-gradient-to-b from-black/60 to-transparent transition-opacity duration-300 ${
      ...
      >
        {/* Render a VideoPlayer for each tab */}
        {tabs.map((tab) => {
          const vRef = getVideoRef(tab.titleId)
          const isActive = tab.titleId === activeTitleId
          return (
            <VideoPlayer
              key={tab.titleId}
              titleId={tab.titleId}
              isActive={isActive}
              activeSub={isActive ? activeSub : null}
              subtitles={isActive ? subtitles : []}
              onTimeUpdate={(t, d) => {
                setCurrentTime(t)
                setDuration(d)
              }}
              ...
              onEnded={() => {}}
              onStreamMode={(mode) => {
                if (tab.titleId === activeTitleId) setStreamMode(mode)
              }}
              videoRef={vRef}
            />
          )
        ...
        {/* Controls overlay */}
        <div
          className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent px-6 py-4 transition-opacity duration-300 ${
        ...
        >
          ...
          <div className="flex items-center gap-4 text-[#e5e2e1]">
            <button
              className="hover:text-[var(--color-accent)] transition-colors text-lg leading-none w-6 flex items-center justify-center"
              onClick={(e) => {
                e.stopPropagation()
                handleTogglePlay()
              }}
            ...
            <div className="relative">
              <button
                className={`text-xs font-mono transition-colors px-1 rounded ${
                  activeSub
                    ? 'text-[var(--color-accent)] bg-[var(--color-accent)]/10'
                    : subtitles.length > 0
                    ? 'text-[#8e9285] hover:text-[#e5e2e1]'
                    : 'text-[#353534] cursor-default'
                }`}
                onClick={(e) => {
                  e.stopPropagation()
                  if (subtitles.length > 0) setShowSubMenu((v) => !v)
                }}
                  ...
                  <button
                    className={`w-full text-left px-3 py-2 text-xs transition-colors ${
                      !activeSub
                        ...
                        : 'text-[#8e9285] hover:text-[#e5e2e1] hover:bg-[#2a2a2a]'
                    }`}
                    onClick={(e) => {
                      e.stopPropagation()
                      setActiveSub(null)
        ...
        {showBrowse && (
          <BrowseOverlay
            ...
            existingIds={tabs.map((t) => t.titleId)}
          />
        )}
      </div>
      ...
      <div className="px-6 py-1 flex gap-6 text-[10px] text-[#353534] font-mono bg-[#0a0a0a] shrink-0">
        {[
          ...
          <span key={key}>
            <span className="text-[#454652]">{key}</span> {desc}
          </span>
        ))}
      </div>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded text-sm min-w-0 shrink-0 transition-colors border-b-2 ${
                isActive
                  ? 'bg-[#1c1b1b] border-[var(--color-accent)]'
            ...
            >
              ...
              <span className="text-[#e5e2e1] font-medium truncate text-xs max-w-[100px]">
                {tab.title}
              </span>
              <span
                role="button"
